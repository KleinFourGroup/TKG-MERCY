...
| | |
|---|---|
| **NEXT** | **Step 83** — Order Status due date + open-orders toggle ([ROADMAP](ROADMAP.md)) — ⏸ **blocked on the team:** ask what "show / hide open orders" means (+ the default state) before building. Then the overhaul block: 85 → 86 → 87. |
| **Last landed** | **Step 84** — the doc split (this structure; `MERGE_PLAN.md` retired to [plan_archive/merge_plan.md](plan_archive/merge_plan.md)). Before it: 82 (whole-piece quantities + H:MM), 81 (`refreshAllViews()`). |
