...
> **🗄️ ARCHIVED 2026-07-17 (Step 84, the doc split).** This was the live plan doc (`MERGE_PLAN.md` at repo root) from 2026-04-16 to 2026-07-17; it is now frozen history. **Live status moved to [`HANDOFF.md`](../HANDOFF.md) (🧭 Cursor) + [`ROADMAP.md`](../ROADMAP.md) + [`WORKLOG.md`](../WORKLOG.md).** `MERGE_PLAN §12.x / §13.x` references in code comments and older docs resolve to the sections below, unchanged. Relative links in the body were written from repo root and are stale from this location (e.g. `plan_archive/foo.md` → `foo.md`, `scheduling.py` → `../scheduling.py`); left as-is since historical accuracy matters more than polish (the standing archive convention). The §12.1 NEXT pointer and "Current smoke baseline" quoted below are frozen at archive time — the Cursor carries the live values.
...
| 83 | 🔲 Planned | Order Status shows due date + a show/hide open-orders toggle on both the Orders and Order Status tabs (team request; UI-only, no schema) — ⏸ **the toggle's meaning is ambiguous, ask the team before building** — see §13.52 |
| 84 | 🔲 Planned | **Doc split — do first.** HANDOFF.md + a single-source live "Cursor", ROADMAP/WORKLOG as separate files, rotate closed rounds to `plan_archive/`; retire `MERGE_PLAN.md` — see §13.53 |
| 85 | 🔲 Planned | Machine-enforce the single-source facts: a smoke check asserting the live doc's smoke baseline == the registered check count — see §13.53 |
| 86 | 🔲 Planned | Make illegal record states unrepresentable (required ctor args + one validating way in) — the root of the `addX()`-vs-dict-insert / None-field edge cases — see §13.53 |
| 87 | 🔲 Planned | Mutation-path + boilerplate sweep (absorbs the former Step 83 `EditWindow` base — rescoped to *every mutation path*, not `*EditWindow`; + named widget fields + one refresh protocol) — see §13.53 |
...
**⏸ Deferred to Step 87 (§13.53; was Step 83 until the 2026-07-16 renumber):** the `EditWindow` base class. B leaves one `refreshAllViews()` call per edit path — inherited-instead-of-written is a pure refactor that stacks on B with no rework, so it was split out rather than entangled with a bugfix.
...
### 13.52 Order status due date + open-orders toggle (planned 2026-07-16) — Step 83

The team's outstanding feature request (via Matthew, 2026-07-16). Two asks, both UI-only, no schema change:
...
**⏸ Ask the team before building — "show / hide open orders" is ambiguous.** It most likely means *filter to open orders only* (hide fulfilled ones, to declutter), but read literally it means hiding the open ones, which is the opposite. **Do not guess** — one question to the team settles it, and the wrong reading ships a filter that hides exactly the rows they wanted. Confirm the default state too (filtered or unfiltered on open).

**Scoping notes (from a 2026-07-16 survey — these are the non-obvious parts):**
...
- **Put the filter in [`utils.py`](utils.py) beside `orderSortCombo` / `orderSortKey` / `ORDER_SORT_*`**, which both tabs already share. Same reasoning as Steps 66/67/82's shared view helpers: the two tabs must agree, and a shared helper is what makes that structural instead of remembered.
- **Decide whether the Step 77 Orders / Order Status PDF report honours the filter.** It's linked from both tabs; Step 82's precedent is that the tab and the PDF should read identically, so a filtered tab printing an unfiltered PDF would be a surprise. Probably yes — but confirm, since it may be exactly the full record they want on paper.
...
### 13.53 Post-release overhaul block (planned 2026-07-16) — Steps 84–87

Matthew, 2026-07-16, after the release: *"the codebase has a lot of weird edge cases that really need to be cleaned up"* and *"MERGE_PLAN is a behemoth… the markdown equivalent of spaghetti code. We need to get a proper agentic workflow up and running."* Both diagnoses are correct and evidenced — see the Step 81 follow-up commit, where a cold-read audit found a shipped bug plus four rebuild traps. **Do these as separate steps, not a cleanup mega-commit** (same reasoning that split option D out of Step 81), and keep smoke green at every step rather than at the end.

**The root diagnosis: this doc is shaped for a phase the project left.** `MERGE_PLAN.md` is named for a merge that completed at **Step 13**; it now carries **83** steps of post-release work driven by team feedback bursts. It was written as a roadmap for planned work and is being used as a changelog for unplanned work. Those have opposite lifecycles — a plan should *shrink* as work lands, a log only *grows* — so the live backlog drowns in as-built history. Concretely: §13 is **789 of 955 lines (83%)**, and the essays have grown steadily (§13.1–13.29 are 4–6-line pointers into [`plan_archive/`](plan_archive/) — the pattern that works; §13.35 = 6 lines, §13.45 = 20, **§13.50 = 42**). This is what hid §13.45's dropdown warning, which had correctly predicted Step 81's one real risk *months* ahead and was still missed at scoping time.

- **Step 84 — the doc split. Do this first; everything after is cheaper.** Adopt the shape proven in Matthew's `asciibattler` project (`~/projects/asciibattler`): a **HANDOFF.md** whose "🧭 Cursor" is *the ONE home for live status* — every other doc and the agent memory **point** at it rather than restating it — plus **ROADMAP** (forward, shrinks) and **WORKLOG** (backward, rotates to archive) as *separate files*. On close, a round condenses to one blockquote of *still-load-bearing facts* and the essay archives wholesale. MERCY already has `plan_archive/`; §13.30+ merely stopped using it. Target: a live surface of ~200 lines. Retire `MERGE_PLAN.md` to the archive — it did its job. **Caution:** don't copy asciibattler's Cursor *density* (~1,500 words in one table cell); it's under the same rot pressure, being one place that only grows.
- **Step 85 — machine-enforce the single-source facts.** Every doc rot found on 2026-07-16 was *one fact stated twice, one copy updated* — including the fix itself, which shipped `71` in one place and `72` in another before a second cold read caught it. A smoke check asserting *"the baseline count in the live doc == the number of registered checks"* makes the convention structural instead of remembered. Extends naturally (no item marked Planned for a step marked Done). Cheap; same move as Step 81 itself.
- **Step 86 — make illegal record states unrepresentable.** *The* root of the "weird edge cases". `Material("Clay")` leaves `Plus50 = None` and the table formatter crashes; `Mixture` needs a `db` back-ref that **only** `addMixture()` sets, so `getCost()` raises on a raw dict insert. Every consumer must defend, the defenses are inconsistent, and this single root produced **three** separate incidents on 2026-07-16 alone (the smoke-fixture bug, a probe crash, and the whole `addX()`-vs-dict-insert convention). Fix direction: required constructor args and one validating way in. Highest leverage of the cleanup.
- **Step 87 — the mutation-path / boilerplate sweep** (absorbs the former Step 83, the `EditWindow` base class — see the note below). **Rescope it: "every mutation path", not `*EditWindow`.** Step 81's follow-up proved the naming convention is *not* the set — `PartsMarginsWindow` mutates `part.price` and sits outside it. Fold in two smaller chores found the same day: name the anonymous widget fields (`mainLayout[2][1]` — brittle, and it broke two probes), and collapse the refresh method-name zoo (`refreshTable`/`refresh`/`refreshTab`/`refreshPicker`/`refreshRows`/`refreshValueLabels`/`refreshSnapshots`/…), which is the only reason `_refreshAllTabs` needs an adapter at all.

**Also worth building (not yet numbered): a `/handoff` skill.** Codify the cold-read audit that worked on 2026-07-16 — spawn a subagent that follows CLAUDE.md's onboarding *cold* and adversarially audits docs against code. It found a shipped inventory bug and four rebuild traps in one pass. It should be a command, not an improvisation.

#### Former Step 83 — shared `EditWindow` base class (now folded into Step 87)
...
**Shape of the work (now part of Step 87) — pure refactor, no behaviour change, no schema change.**
