# MERCY — Original merge design (§§1–11, archived)

*Moved verbatim from `MERGE_PLAN.md` §§1–11 on 2026-06-24, to keep the live plan under the single-read budget after the Production Scheduling step series (Steps 42–54) landed. This is the original ANIKA + BECKY + production-tracker merge design — Steps 1–13, complete 2026-04. Preserved verbatim: internal `§` cross-references resolve within this document (e.g. "§3.6", "§7.1", "§10"), except "§12"/"§13" which point to `MERGE_PLAN.md` for live status. For current project state see `MERGE_PLAN.md`; for per-step implementation narratives see `implementation_notes.md`.*
