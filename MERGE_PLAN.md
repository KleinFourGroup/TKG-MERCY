...
- **File-dialog directory memory.** The Open / Save-As / Import dialogs ([`app.py`](app.py) ~lines 226 / 246 / 299) all hardcode `os.path.expanduser("~")` as their start dir, ignoring where the user last worked. Seed each from a remembered last-used directory via `QSettings`, mirroring the Step 20 `lastDbPath` pattern. (§13.7 flagged only the Import path; it's really all three dialogs.)
- **"App Info" Settings sub-tab.** The §7.1 App Info (version / about) sub-tab was skipped in Step 5 as trivial polish ("add whenever convenient" — [`plan_archive/implementation_notes.md`](plan_archive/implementation_notes.md) Step 5). Small.
