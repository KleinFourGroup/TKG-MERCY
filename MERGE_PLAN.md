...
**Next up (fresh session): Step 65** — Presser → Press preference, the first *reuse* of the Step 64 grid, and the only step in this block that touches the schema (`presser_press_pref` table, additive migration **db_version 11→12**). It's a full §13.30 vertical slice, so mind the migration-version-churn upkeep note above (terminal-version literals + docstrings in `smoke/migrations.py`, register the new CRUD check in both `smoke/__init__.py` and `smoke/__main__.py`, new table via migration + fresh-schema path only — never `UNIFIED_TABLES`).
