...
| 74b | ⬜ Planned | Trucks-mode Order Status entry ("Enter in trucks" checkbox; converts via `part_truck`, stores/shows pieces) — see §13.46 |
...
**Handoff — remaining in this block: Step 74b (trucks-mode entry), a fresh session.** UI-only, consumes the now-shipped `part_truck`. **74b** = a live, unpersisted **"Enter in trucks"** checkbox on the Order Status snapshot editor ([`order_status_tab.py`](order_status_tab.py)) that reinterprets the remaining-to-press/-ship fields as half-trucks and converts to pieces via `db.partTruck[part].partsPerTruck` before storing (stored + displayed always in pieces), **blocking** when the part has no parts-per-truck set or a half-truck of an odd count wouldn't land on whole pieces. Half-truck precision via a step-0.5 input widget. The exact block-on-unset error UX (inline vs. on-commit) is to be settled at 74b. Manual UI gate.
