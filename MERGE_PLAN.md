...
**Pairs with §13.45** (validate the greedy against real order data) — still open: the die-placement seed is now the real starting state that validation needs, but real orders/presses must be entered first.
...
Two findings from Matthew's first look at a **real-order** schedule (2026-07-16, the DB now has actual order data — and the Step 80 die stickiness "looks great", so that half is confirmed on the floor's real data):
