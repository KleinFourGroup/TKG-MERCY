...
| 34 | 📝 Sketched | Dead-code sweep with vulture — see §13.22 |
| 35 | 📝 Sketched | Smoke: render every report against fuzzed data — see §13.23 |
| 36 | 📝 Sketched | Pyright sweep across the codebase — see §13.24 |
...
### 13.22 Step 34 — dead-code sweep with vulture 📝 Sketched

Run [`vulture`](https://github.com/jendrikseipp/vulture) over the codebase to flag candidate unused functions, classes, attributes, and imports. MERCY's BECKY+ANIKA merge history makes this high-yield: vestigial code has surfaced organically more than once already (Step 9.5's `Part` attribute cleanup, the unused `import sqlite3` in old `records.py`, the unused `from math import floor` noticed mid-Step-33). Vulture would catch that class of waste automatically rather than relying on someone noticing.

**Scope (one focused step):** (a) `pip install vulture`, add to a dev section of requirements.txt. (b) Run with `--min-confidence 80` to keep noise low. (c) Triage findings: delete confirmed dead code; add `# noqa` / vulture-whitelist entries for false positives (Qt slots connected via `signal.connect()` are the dominant noise source — they look unused but are wired at runtime). (d) Commit the cleanup. (e) Decide whether to bake vulture into a recurring lint or treat this as a one-time hygiene pass.

**Why first:** lowest cost, fastest win. Push-button after the initial triage. Likely surfaces 3-5 real cleanup wins given the merge history.

### 13.23 Step 35 — smoke: render every report against fuzzed data 📝 Sketched

Add a smoke check that builds a `tiny`-scale fuzz DB via `fuzz_db.py`, instantiates `PDFReport`, calls every public report method with reasonable arguments, and asserts each output file is non-empty + starts with the `%PDF-` magic. The product reports (`globalsReport` / `mixReport` / `salesReport` / `inventoryReport`) and employee reports (`employeePointsReport` / `employeePTOReport` / `employeeNotesReport` / `employeeIncidentReport` / `employeeActiveReport`) are **not** currently in smoke — Step 33 needed a manual UI sweep to gate them. This check would have automated that gate.

**Scope (one focused step):** (a) New check function in `smoke/reports.py` looping the report API. (b) Pick one employee, one mix, one part, one inventory date for the parameterized reports (use the deterministic-seed fuzz DB so the same names exist every run). (c) Use the same `tempfile`-based output pattern existing report checks use — write to a temp dir, validate magic bytes, clean up. (d) Confirm runtime delta is acceptable (a few seconds at most expected at `tiny` scale).

**Tradeoff:** more maintenance surface to keep green, but every future refactor of `report/` (including the eventual one-report-per-mixin split from §13.21) gets automatic regression coverage instead of a manual UI gate.

### 13.24 Step 36 — Pyright sweep across the codebase 📝 Sketched

Pylance (the VS Code editor type-check engine) already flags issues smoke can't see — `employee.idNum possibly None`-style warnings Matthew has been spotting manually as he reads code. Pylance is editor-only, but it's built on [**Pyright**](https://github.com/microsoft/pyright), which has a CLI: `pip install pyright`, then `pyright .` from the repo root.

**Scope (likely 1-2 sessions, not single-commit):** (a) Add Pyright to requirements.txt; write a `pyrightconfig.json` that excludes `Lib/site-packages/`, `Scripts/`, and the build artifacts. (b) Run an initial sweep — expect a sizable backlog given the BECKY+ANIKA merge history (a lot of `Optional[X]` getters never null-checked at the call site). (c) Triage by category: Optional unwrapping, missing return types, attribute-on-None patterns. (d) Decide per-category whether to fix, suppress with `# pyright: ignore[<rule>]`, or relax the strictness setting on a per-file basis. (e) Once the baseline is clean, bake `pyright .` into the always-on smoke baseline so regressions are caught at PR time.

**Why third (after dead code + smoke):** the initial backlog triage is real work, not a one-commit step. Highest-value precondition: dead code already removed (Step 34) so we're not type-checking soon-to-be-deleted code, and smoke coverage already broad (Step 35) so behavior regressions during the type-check fixes are caught immediately.
