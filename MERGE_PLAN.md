...
- **~~"App Info" Settings sub-tab~~ → re-scope as an optional About / Diagnostics panel.** The §7.1 App Info (version / about) sub-tab was skipped in Step 5 as trivial polish. **As-scoped it's now redundant and should be dropped** — the app name + version already live in the title bar ([`app.py`](app.py) `setWindowTitle`), so a literal version/about tab just duplicates it (call, Matthew + Claude, 2026-07-02). The only version worth building is a **support/diagnostics** panel showing what the title bar *doesn't*: the open DB's schema `db_version` (currently 12 — the actually-useful migration-support diagnostic, nowhere visible in the UI today), the full build id (git hash/dirty from [`version.py`](version.py)), and the open DB path / `QSettings` location. Build only if the team hits support friction — it's a different, better feature than the original note, not a tonight item.
...
- **"Lever 1" — one-registration-list downstream refresh.** Route every edit/delete success path through `MainWindow._refreshAllTabs()`, collapsing the N×M "remember to refresh every downstream tab" wiring obligation into a single registration list — killing the stale-view bug *class* at the source (detection / "Lever 2" already shipped as the Step 55 net, §13.31). **⚠ Larger than it looks (flagged by Matthew, 2026-07-02):** several tabs render off a drop-down / selection that does **not** persist across a refresh — the production tab (employee + date filters), employee overview / detail, inventory (date selection) — which is exactly why they're *excluded* from the Step 55 projection registry. A blanket `_refreshAllTabs()` would reset the user's current filter/selection on those tabs, so Lever 1 first requires reworking those combo-driven tabs to refresh **selection-preservingly** (per-tab work), not just re-pointing the wiring. Scope it as a multi-step effort, not a one-commit rewire.
...
**Handoff — remaining in this block: Steps 74a/74b (parts-per-truck), a fresh session.** The only schema step in §13.46. All four design calls are settled above; the build shape is the §13.30 vertical-slice template: **74a** = `part_truck(part PRIMARY KEY, partsPerTruck INTEGER)` + `_createSchedulingTables` + additive `_migrateV12ToV13` (**db_version 12→13**, mind the migration-churn upkeep note above) + record/save/load + `database.py` dict + setter + **part rename/delete cascades** (mirror the Step 48 part-press-pref cascades) + `fuzz_db` populator + a **Parts per Truck** tab under Scheduling config + smoke CRUD + crash_fuzz dispatcher + projection-tab registry; **74b** = a live, unpersisted **"Enter in trucks"** checkbox on the Order Status snapshot editor ([`order_status_tab.py`](order_status_tab.py)) that reinterprets the remaining-to-press/-ship fields as half-trucks and converts to pieces via `part_truck` before storing (stored + displayed always in pieces), **blocking** when the part has no parts-per-truck set or a half-truck of an odd count wouldn't land on whole pieces. Manual UI gate on both.
