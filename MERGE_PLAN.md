...
**Next up: Step 67** — opens with the Date/Shift→subheader regroup above (the width fix + grouped base layout), **then** adds the two named report actions: a **per-shift** schedule and a **date-range-limited** schedule (a single action may take both, per the team's "third shift, next three days" example). Both **filter the full computed `ScheduleResult`** for display/PDF — a view slice, not a shortened scheduler horizon. Pure UI on [`schedule_tab.py`](schedule_tab.py) + [`report/scheduling.py`](report/scheduling.py); manual UI gate. Closes the Steps 63–67 block.
