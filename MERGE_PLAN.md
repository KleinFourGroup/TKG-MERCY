...
**Next up (fresh session): Step 66** — **Scheduler assigns pressers (secondary).** The big one in this block, and logic-first (like Steps 51/52), so the manual gate is only a glance. `ScheduleRow` gains `presser: int | None`. After `_assignLanes` fixes which presses run and what they press (unchanged, part-preference-driven), a new matching pass staffs the present pressers (`pressersPresent`) onto those presses maximizing total presser→press score (Not set = `presserNeutralScore`, deterministic tiebreak). `ScheduleConfig` gains `presserNeutralScore = 3` and a `presserAssignment = "preference"` policy seam (`"balanced"` / `rotationWeight` reserved-but-unimplemented, the §10 provisional-heuristic pattern). Thread a Presser column through [`schedule_tab.py`](schedule_tab.py) + the [`report/scheduling.py`](report/scheduling.py) PDF, and append a short presser-heuristic note to [`prod-sched-algorithm.md`](plan_archive/prod-sched-algorithm.md). **No schema change** (the scheduler is stateless — no `db_version` bump). *Invariant (design call 2026-07-02):* presser preference never changes *what* is produced or *which* presses run — only *who stands where*; normal case is pressers < presses (nobody starved), the rare inverse leaves surplus pressers unassigned that shift. Now that Step 65 shipped the data, Step 66 is the consumer — the `PresserPressPref.getScore(press)` / `db.presserPressPref` reads and the `scheduling._prefScore`-style neutral-midpoint fallback are the seams to build on.
