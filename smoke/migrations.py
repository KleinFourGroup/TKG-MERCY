...
the unified-MERCY v3->v4 (hours column), v4->v5 (presses table) and
v5->v6 (pressers table) migrations."""
import glob
import os
import sys
import tempfile
...
def legacy_anika_migration() -> list[str]:
    ...
    Expected post-open state:
      - db_version = 6 (Case 3 stamps MERCY_DB_VERSION after the ANIKA normalization;
    ...
    try:
        ...
        if version is None or int(version[0]) != 6:
            errors.append(f"db_version expected 6, got {version}")
...
def mercy_v3_to_v4_migration() -> list[str]:
    ...
    data is preserved with hours=0. (The chain continues to v6; terminal
    db_version is asserted as 6, and the v4->v5 presses / v5->v6 pressers
    tables land too.)"""
    from PySide6.QtWidgets import QApplication
    from app import MainWindow
    import sqlite3
    ...
    try:
        ...
        version = conn.execute("SELECT value FROM globals WHERE name='db_version'").fetchone()
        if version is None or int(version[0]) != 6:
            errors.append(f"post-migration db_version expected 6, got {version}")
...
def mercy_v4_to_v5_migration() -> list[str]:
    ...
    existing production data survives untouched. (The chain continues to v6;
    terminal db_version is asserted as 6, and the v5->v6 pressers table lands
    too.)"""
    from PySide6.QtWidgets import QApplication
    from app import MainWindow
    import sqlite3
...
def mercy_v5_to_v6_migration() -> list[str]:
    ...
    created, db_version is bumped to 6, and existing presses / production data
    survive untouched."""
    from PySide6.QtWidgets import QApplication
    from app import MainWindow
    import sqlite3
