...
v5->v6 (pressers table) and v6->v7 (shift_workweek table) migrations."""
import glob
import os
import sys
import tempfile
...
def legacy_anika_migration() -> list[str]:
    ...
    Expected post-open state:
      - db_version = 7 (Case 3 stamps MERCY_DB_VERSION after the ANIKA normalization;
    ...
    try:
        ...
        if version is None or int(version[0]) != 7:
            errors.append(f"db_version expected 7, got {version}")
...
def mercy_v3_to_v4_migration() -> list[str]:
    ...
    data is preserved with hours=0. (The chain continues to v7; terminal
    db_version is asserted as 7, and the v4->v5 presses / v5->v6 pressers /
    v6->v7 shift_workweek tables land too.)"""
    from PySide6.QtWidgets import QApplication
    from app import MainWindow
    import sqlite3
    ...
    try:
        ...
        version = conn.execute("SELECT value FROM globals WHERE name='db_version'").fetchone()
        if version is None or int(version[0]) != 7:
            errors.append(f"post-migration db_version expected 7, got {version}")
...
def mercy_v4_to_v5_migration() -> list[str]:
    ...
    existing production data survives untouched. (The chain continues to v7;
    terminal db_version is asserted as 7, and the v5->v6 pressers / v6->v7
    shift_workweek tables land too.)"""
    from PySide6.QtWidgets import QApplication
    from app import MainWindow
    import sqlite3
...
def mercy_v5_to_v6_migration() -> list[str]:
    ...
    created and db_version reaches 7 (the chain continues through v6->v7), with
    existing presses / production data surviving untouched."""
    from PySide6.QtWidgets import QApplication
    from app import MainWindow
    import sqlite3
...
def mercy_v6_to_v7_migration() -> list[str]:
    ...
    shift_workweek table is created, db_version is bumped to 7, and existing
    presses / pressers / production data survive untouched."""
    from PySide6.QtWidgets import QApplication
    from app import MainWindow
    import sqlite3
